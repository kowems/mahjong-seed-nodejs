...
function checkCanZuoChi(seatData,targetPai){
    var value = targetPai % 9;
    ...
    if(seatData.countMap[targetPai+1] > 0 && seatData.countMap[targetPai+2] > 0){
        seatData.canZuoChi = true;
    }

    return seatData.canZuoChi;
}

function checkCanYouChi(seatData,targetPai){
    var value = targetPai % 9;
    ...
    if(seatData.countMap[targetPai-1] > 0 && seatData.countMap[targetPai-2] > 0){
        seatData.canYouChi = true;
    }

    return seatData.canYouChi;
}

function checkCanZhongChi(seatData,targetPai){
    var value = targetPai % 9;
    ...
    if(seatData.countMap[targetPai-1] > 0 && seatData.countMap[targetPai+1] > 0){
        seatData.canZhongChi = true;
    }

    return seatData.canZhongChi;
}
...
function hasOperations(seatData){
    if(seatData.canGang || seatData.canPeng || seatData.canHu ||
       seatData.canZuoChi || seatData.canZhongChi || seatData.canYouChi){
        return true;
    }
    return false;
}
...
function FindNextUser(game,nextSeat){
    var turn = 0;
    ...
    if(nextSeat == null){
        while(true){
            turn = game.turn + 1;
            turn %= 4;
            var turnSeat = game.gameSeats[turn];
            if(turnSeat.hued == false){
                return turn;
            }
        }
...
exports.begin = function(roomId) {
    ...
    for(var i = 0; i < 4; ++i){
        ...
        data.canPeng = false;
        //是否可以左吃
        data.canZuoChi = false;
        //是否可以中吃
        data.canZhongChi = false;
        //是否可以右吃
        data.canYouChi = false;
        //是否可以胡
        data.canHu = false;
        //是否可以出牌
